...
var Actions = (function () {
    ...
    */
    ...
    Actions.prototype.deploy = function (version, sites, options) {
        var _this = this;
        var self = this;
        self._showKadiraLink();
        var getDefaultBuildDirName = function (appName, version) {
            if (appName === void 0) { appName = null; }
            if (version === void 0) { version = null; }
            return (appName || "meteor-") + "-" + (version || uuid.v4());
        };
        var buildLocation = process.env.METEOR_BUILD_DIR || path.resolve(os.tmpdir(), getDefaultBuildDirName(this.config.appName, version));
        ...
        build_1.buildApp(appPath, meteorBinary, buildLocation, bundlePath, function (err) {
            if (err) {
                process.exit(1);
                return;
            }
            ...
            }, _this.whenAfterDeployed(buildLocation, options));
        });
    };
    ...
    * code.
    */
    Actions.prototype.whenAfterCompleted = function (error, summaryMaps) {
    ...
    Actions.prototype.whenSuccess = function (error, summaryMaps) {
    };
    Actions.prototype.whenFailure = function (error, summaryMaps) {
    };
    /**
     * Return a callback, which is used when after deployed, clean up the files.
     */
    Actions.prototype.whenAfterDeployed = function (buildLocation, options) {
        var _this = this;
        return function (error, summaryMaps) {
            ...
            _this.whenAfterCompleted(error, summaryMaps);
        };
    };
    return Actions;
}());
