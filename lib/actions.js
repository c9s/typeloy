...
var linux_1 = require("./taskLists/linux");
var sunos_1 = require("./taskLists/sunos");
...
var ActionsRegistry = (function () {
    function ActionsRegistry(config, cwd) {
        this.cwd = cwd;
        this.config = config;
    ...
    */
    ActionsRegistry.prototype._createSessionsMap = function (config) {
        var sessionsMap = {};
    ...
    ActionsRegistry.prototype._showKadiraLink = function () {
        ...
        if (fs.existsSync(versionsFile)) {
            ...
            if (!hasKadira) {
                console.log("“ Checkout " + "Kadira".bold + "!" +
                    "\n  It's the best way to monitor performance of your app." +
                    "\n  Visit: " + "https://kadira.io/mup".underline + " ”\n");
            }
        }
    };
    ActionsRegistry.prototype._executePararell = function (actionName, args) {
        var self = this;
    ...
    ActionsRegistry.prototype.setup = function () {
        this._showKadiraLink();
        this._executePararell("setup", [this.config]);
    };
    ActionsRegistry.prototype.deploy = function (options) {
        ...
        build_1.buildApp(appPath, meteorBinary, buildLocation, function (err) {
            if (err) {
                process.exit(1);
                return;
            }
    ...
    ActionsRegistry.prototype.reconfig = function () {
        var self = this;
        var sessionInfoList = [];
        for (var os in self.sessionsMap) {
            ...
            sessionsInfo.sessions.forEach(function (session) {
                var env = _.extend({}, self.config.env, session._serverConfig.env);
                var taskList = sessionsInfo.taskListsBuilder.reconfig(env, self.config.appName);
    ...
    ActionsRegistry.prototype.restart = function () {
        this._executePararell("restart", [this.config.appName]);
    };
    ActionsRegistry.prototype.stop = function () {
        this._executePararell("stop", [this.config.appName]);
    };
    ;
    ActionsRegistry.prototype.start = function () {
        this._executePararell("start", [this.config.appName]);
    };
    ActionsRegistry.prototype.logs = function (options) {
        var self = this;
    ...
    ActionsRegistry.prototype.init = function () {
