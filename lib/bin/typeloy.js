...
prog.command('get-login [site]')
    ...
    var siteConfig = config.sites[site];
    if (!siteConfig) {
        console.error(site + " config not found.");
        return;
    }
    var cmds = _.map(siteConfig.servers, function (server) {
        var cmd = ["ssh"];
        if (server.pem) {
            cmd.push('-i');
            cmd.push(server.pem);
...
prog.command('mongo:restore [site] [file]')
    ...
    .action(function (site, file, options) {
    var config = config_1.readConfig(prog.config);
    var a = new actions_1.MongoRestoreAction(config);
    var deployment = Deployment_1.Deployment.create(config, config.app.root || cwd);
    var afterSetup = a.run(deployment, site, file);
    afterSetup.then(function (mapResult) {
        console.log(SummaryMapConsoleFormatter_1.SummaryMapConsoleFormatter.format(mapResult));
        var errorCode = SummaryMap_1.haveSummaryMapsErrors(mapResult) ? 1 : 0;
    }).catch(function (err) {
