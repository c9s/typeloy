#!/usr/bin/env node
"use strict";
var path = require('path');
var config_1 = require('../src/config');
var actions_1 = require('../src/actions');
var Deployment_1 = require('../src/Deployment');
require('colors');
var prog = require('commander');
var version = '1.3.0';
var cwd = path.resolve('.');
prog.version(version);
prog.usage('[options] <subcommand> ...');
prog.option('-v, --verbose', 'verbose mode');
prog.option('-c, --config <file>', 'config file');
prog.command('deploy [site]')
    .description('set the deployment tag and start deploying.')
    .option("-d, --dryrun", 'do not really deploy it.')
    .option("--bundle-file <file>", 'the bundle file you have already built with meteor build.')
    .option("--build-dir <dir>", 'the meteor build directory.')
    .option("--tag <tag>", 'deployment tag')
    .option("-C, --no-clean", 'whether to clean up the bundle files.')
    .action(function (site, options) {
    var config = config_1.readConfig(prog.config);
    var a = new actions_1.DeployAction(config, cwd);
    var deployment = Deployment_1.Deployment.create(config, cwd, options.tag);
    var afterDeploy = a.run(deployment, site, options);
    afterDeploy.then(function (mapResult) {
        console.log("After deploy", mapResult);
        console.log(JSON.stringify(mapResult, null, "  "));
        // var errorCode = haveSummaryMapsErrors(mapResult) ? 1 : 0;
    });
    afterDeploy.catch(function (res) {
        console.error(res);
    });
});
prog.command('setup [site]')
    .description('setup the requirements on the target server.')
    .action(function (site, options) {
    var config = config_1.readConfig(prog.config);
    var a = new actions_1.SetupAction(config, cwd);
    var deployment = Deployment_1.Deployment.create(config, cwd);
    a.run(deployment, site);
});
prog.command('logs [site]')
    .description('tail the logs')
    .option("-f, --tail", 'tail')
    .action(function (site, options) {
    var config = config_1.readConfig(prog.config);
    var a = new actions_1.LogsAction(config, cwd);
    var deployment = Deployment_1.Deployment.create(config, cwd);
    a.run(deployment, site, options);
});
prog.command('start [site]')
    .description('start the app.')
    .action(function (site, options) {
    var config = config_1.readConfig(prog.config);
    var actions = new actions_1.StartAction(config, cwd);
    var deployment = Deployment_1.Deployment.create(config, cwd);
    actions.run(deployment, site);
});
;
prog.command('stop [site]')
    .description('stop the app.')
    .action(function (site, options) {
    var config = config_1.readConfig(prog.config);
    var actions = new actions_1.StopAction(config, cwd);
    var deployment = Deployment_1.Deployment.create(config, cwd);
    actions.run(deployment, site);
});
;
prog.command('restart')
    .description('restart the app.')
    .action(function (site, options) {
    var config = config_1.readConfig(prog.config);
    var actions = new actions_1.RestartAction(config, cwd);
    var deployment = Deployment_1.Deployment.create(config, cwd);
    actions.run(deployment, site);
});
;
prog.command('init')
    .description('init the mup.json config.')
    .action(function (env, options) {
    var config = config_1.readConfig(prog.config);
    var actions = new actions_1.BaseAction(config, cwd);
    actions.init();
});
/*
// handling undefined command
prog.command('*')
  .action(function(env){
    console.log('deploying "%s"', env);
  });
*/
prog.on('--help', function () {
    /*
    console.log('  Examples:');
    console.log('');
    console.log('    $ custom-help --help');
    console.log('    $ custom-help -h');
    console.log('');
    */
});
prog.parse(process.argv);
//# sourceMappingURL=typeloy.js.map