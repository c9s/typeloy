...
var PluginRunner = (function () {
    ...
    PluginRunner.prototype.whenSuccess = function (deployment) {
        ...
        var promises = _.map(this.plugins, function (p) { return p.whenSuccess(deployment); }).compact();
    ...
    PluginRunner.prototype.whenFailure = function (deployment) {
        ...
        var promises = _.map(this.plugins, function (p) { return p.whenFailure(deployment); }).compact();
        if (promises.length == 0) {
            return;
        }
        return Promise.all(promises);
