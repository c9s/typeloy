...
class PluginRunner {
    ...
    whenBeforeBuilding(deployment) {
        underscore_1.default.each(this.plugins, (p) => { p.whenBeforeBuilding(deployment); });
    }
    whenBeforeDeploying(deployment) {
        underscore_1.default.each(this.plugins, (p) => { p.whenBeforeDeploying(deployment); });
    }
    ...
    whenAfterCompleted(deployment) {
        underscore_1.default.each(this.plugins, (p) => { p.whenAfterCompleted(deployment); });
    }
    whenAfterDeployed(deployment) {
        underscore_1.default.each(this.plugins, (p) => { p.whenAfterDeployed(deployment); });
    }
}
...
class SlackNotificationPlugin extends Plugin_1.Plugin {
    ...
    createGitCommitAttachment(revInfo, extra) {
        ...
        return underscore_1.default.extend(attachment, extra || {});
    }
    /**
     * @return {deferred.promise}
     */
    send(msg) {
        return this.api.send(underscore_1.default.extend({
