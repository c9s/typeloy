...
var LinuxTaskBuilder = (function (_super) {
    ...
    LinuxTaskBuilder.prototype.reconfig = function (env, config) {
        var taskList = this.taskList("Reconfiguring Application (linux)");
        var tasks = [];
        tasks.push(new tasks_1.EnvVarsTask(config, env));
    ...
    LinuxTaskBuilder.prototype.mongoGet = function (config, file) {
        ...
        var taskList = this.taskList("MongoDB Get (linux)");
        tasks.forEach(function (t) {
            t.build(taskList);
        });
        return taskList;
    ...
    LinuxTaskBuilder.prototype.mongoRestore = function (config, localFile) {
        var tasks = [];
        if (config.mongo) {
            tasks.push(new tasks_1.UploadTask(config, localFile, '/tmp/mongo-restore-' + uuid.v4(), true));
            tasks.push(new tasks_1.StopTask(config));
            tasks.push(new tasks_1.MongoRestoreTask(config, '/tmp/mongo-restore-' + uuid.v4()));
            tasks.push(new tasks_1.StartTask(config));
        }
        else {
            console.error("mongo settings is not configured.");
        }
        var taskList = this.taskList("MongoDB Restore (linux)");
        tasks.forEach(function (t) {
            t.build(taskList);
        });
        return taskList;
    ...
    LinuxTaskBuilder.prototype.mongoDump = function (config) {
        ...
        var taskList = this.taskList("MongoDB Dump (linux)");
        tasks.forEach(function (t) {
            t.build(taskList);
        });
        return taskList;
    ...
    LinuxTaskBuilder.prototype.logs = function (config, hostPrefix, logOptions) {
        ...
        var taskList = this.taskList("Getting Application Log (linux)");
        tasks.forEach(function (t) {
            t.build(taskList);
        });
        return taskList;
