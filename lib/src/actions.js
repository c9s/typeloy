...
function getTaskBuilderByOs(os) {
    switch (os) {
        case "linux":
            return LinuxTaskBuilder_1.default;
        case "sunos":
            return SunOSTaskBuilder_1.default;
        default:
...
class Actions {
    ...
    */
    _createSiteSessionsMap(config, siteName) {
        var sessionsMap = {};
        if (!siteName) {
            siteName = "_default_";
        }
        config.sites[siteName].servers.forEach((server) => {
            let session = SessionManager_1.SessionManager.create(server);
