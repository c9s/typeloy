...
function haveSummaryMapsErrors(summaryMaps) {
    return underscore_1.default.some(summaryMaps, hasSummaryMapErrors);
}
function hasSummaryMapErrors(summaryMap) {
    return underscore_1.default.some(summaryMap, (summary) => {
...
class Actions {
    ...
    */
    ...
    _executePararell(actionName, deployment, args) {
        var self = this;
        var sessionInfoList = underscore_1.default.values(self.sessionsMap);
    ...
    deploy(deployment, sites, options) {
        ...
        build_1.buildApp(appPath, meteorBinary, buildLocation, bundlePath, () => {
            ...
            var sessionsData = [];
            underscore_1.default.forEach(self.sessionsMap, (sessionsInfo) => {
                var taskListsBuilder = sessionsInfo.taskListsBuilder;
                underscore_1.default.forEach(sessionsInfo.sessions, (session) => {
                    sessionsData.push({
            ...
            async.mapSeries(sessionsData, (sessionData, callback) => {
                ...
                var env = underscore_1.default.extend({}, this.config.env, session._serverConfig.env);
                // Build deploy tasks
