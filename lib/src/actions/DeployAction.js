...
var DeployAction = (function (_super) {
    ...
    DeployAction.prototype.run = function (deployment, sites, options) {
        ...
        var appName = this.config.app.name || "app";
        ...
        return builder.buildApp(this.config.app.directory, buildLocation, bundlePath, function () {
            ...
            var _loop_1 = function(i) {
                ...
                sitesPromise = sitesPromise.then(function (previousSummaryMap) {
                    ...
                    var meteorSettings = _.extend({
                        ...
                        "private": {},
                        "log": { "level": "warn" }
                    }, siteConfig.settings || _this.config.app.settings || {}); // prefer site config over the app settings
                    // always update
                    if (_this.config.deploy.exposeSiteName && meteorSettings['public'] && typeof meteorSettings['public']['site'] === "undefined") {
                    ...
                    var groupPromises = _.map(sessionsMap, function (sessionGroup) {
                        ...
                        var sessionPromises = _.map(sessionGroup.sessions, function (session) {
                            return new Promise(function (resolveTask) {
                                ...
                                var taskList = taskBuilder.deploy(_this.config, bundlePath, env);
