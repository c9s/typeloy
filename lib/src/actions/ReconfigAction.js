...
var ReconfigAction = (function (_super) {
    ...
    ReconfigAction.prototype.run = function (deployment, sites) {
        ...
        var self = this;
        var sessionInfoList = [];
        var sitesPromise = _.map(sites, function (site) {
            ...
            var _loop_1 = function(os) {
                ...
                sessionGroup.sessions.forEach(function (session) {
                    ...
                    var taskList = taskListsBuilder.reconfig(env, _this.config.app.name);
        ...
        return Promise.all(sitesPromise).then(function (sitesResult) {
            var mapResult = _.flatten(sitesResult);
