...
class ConfigParser {
    ...
    static preprocess(config) {
        ...
        underscore_1.default.each(config.sites, (siteConfig, siteName) => {
            underscore_1.default.each(siteConfig.servers, (server) => {
    ...
    static validate(config) {
        ...
        if ((config.servers instanceof Array && config.servers.length == 0)
            || underscore_1.default.isEmpty(config.sites)) {
